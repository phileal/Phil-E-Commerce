...
export default function CustomerReviews() {
    ...
    const filteredReviews = reviews.filter((rev) => {
        const matchesSearch = reviewName(rev).toLowerCase().includes(searchTerm.toLowerCase()) ||
            reviewText(rev).toLowerCase().includes(searchTerm.toLowerCase());
        const matchesRating = ratingFilter ? rev.rating === Number(ratingFilter) : true;
    ...
    return (
        <div className="p-6 bg-gray-900 min-h-screen text-gray-100">
            <div className="flex items-center justify-between mb-6">
                ...
                <Link to="/AdminDashboard" className="text-sm bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded">
                    ← Back to Dashboard
                </Link>
            </div>
            ...
            {filteredReviews.length === 0 ? (
                ...
                <ul className="space-y-3 max-h-[70vh] overflow-y-auto">
                    {filteredReviews.map((rev, i) => (
                        <li key={i} className="bg-gray-800 p-4 rounded shadow flex justify-between items-start">
                            <div>
